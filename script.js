...
const minimseQuickstartButton = document.getElementById(
  "minimseQuickstartButton"
);
const restoreQuickstartButton = document.getElementById(
  "restoreQuickstartButton"
);
...
//the form inputs
const allTheFormInputs = document.querySelectorAll(
...
//the form3 inputs
const allTheFormInputsEdit = document.querySelectorAll(
  "#theForm3 input[type='number']"
);
...
const decrementWorkPeriodBtnEdit = document.getElementById(
  "decrementWorkPeriodEdit"
);
const incrementWorkPeriodBtnEdit = document.getElementById(
  "incrementWorkPeriodEdit"
);

const decrementRestPeriodBtnEdit = document.getElementById(
  "decrementRestPeriodEdit"
);
const incrementRestPeriodBtnEdit = document.getElementById(
  "incrementRestPeriodEdit"
);
...
const presetsHeaderContainer = document.getElementById(
  "presetsHeaderContainer"
);
...
decrementWorkPeriodBtnEdit.addEventListener(
  "click",
  minusOneFromWorkPeriodEdit
);
incrementWorkPeriodBtnEdit.addEventListener("click", plusOneToWorkPeriodEdit);

decrementRestPeriodBtnEdit.addEventListener(
  "click",
  minusOneFromRestPeriodEdit
);
...
function getAndDisplayExistingPresets() {
  console.log(
    `There are ${localStorage.length} existing presets in local storage to show`
  );
  ...
  arrayOfSavedPresetNames.forEach((arrayElement) => {
    console.log(arrayElement);

    let savedPresetString =
      localStorage.getItem(arrayElement); /*this is a string in the format:
...
function savePresetToLocalStorage(
  ...
  console.log(
    `This '${whatToSave}' is what will be in local storage under the name '${presetNameToSave}'`
  );
...
function insertNewSavedPreset(
  ...
  let totalTimeInSeconds =
    5 +
    transformedSets * workTimePerSetInSeconds +
    (transformedSets - 1) * restTimePerSetInSeconds;

  //now convert that into mins and secs using the remainder/modulus
  let totalTimeSecsRemainder = totalTimeInSeconds % 60;
  let totalTimeMinsQuotient =
    (totalTimeInSeconds - totalTimeSecsRemainder) / 60;
  ...
  if (transformedworkMins < 10) {
    transformedworkMins = `0`.concat(transformedworkMins);
  }
  if (transformedworkSecs < 10) {
    transformedworkSecs = `0`.concat(transformedworkSecs);
  }

  if (transformedrestMins < 10) {
    transformedrestMins = `0`.concat(transformedrestMins);
  }
  if (transformedrestSecs < 10) {
    transformedrestSecs = `0`.concat(transformedrestSecs);
  }

  if (totalTimeSecsRemainder < 10) {
    totalTimeSecsRemainder = `0`.concat(totalTimeSecsRemainder);
  }
  if (totalTimeMinsQuotient < 10) {
    totalTimeMinsQuotient = `0`.concat(totalTimeMinsQuotient);
  }
...
function writeIntoPrepareScreen(mins, secs) {
  if (mins >= 10 && secs >= 10) {
    //i.e. if both don't need to have a 0 added to the front
    prepMins.innerHTML = mins;
    prepSecs.innerHTML = secs;
  } else if (mins >= 10 && secs < 10) {
    //i.e. if only secs needs to have a 0 added to the front
    prepMins.innerHTML = mins;
    prepSecs.innerHTML = "0" + secs;
  } else if (mins < 10 && secs >= 10) {
    //i.e. if only mins needs to have a 0 added to the front
    prepMins.innerHTML = "0" + mins;
    prepSecs.innerHTML = secs;
  } else {
    //i.e. if both need to have a 0 added to the front
    prepMins.innerHTML = "0" + mins;
    prepSecs.innerHTML = "0" + secs;
  }
}
...
function startPrepareScreenCountDown(mins, secs) {
  ...
  console.log(
    `prep countdown starts one second after ${timePrepCountdownStarts}`
  );
  ...
  const x = setInterval(() => {
    //i.e every second, do the following
    ...
    if (currentTimeInMs < newMs) {
      ...
      if (mins == 0) {
        /*the simplest case i.e if prep time is less than a min, just decrement the secs arg and write into the html*/
        writeIntoPrepareScreen(mins, secs);
        secs--;
      } else {
        //i.e how to handle longer prep times eg 2mins30secs
      ...
      //what to do at the end of the timer
...
function getAndShowWorkScreen() {
  ...
  console.log(
    `Sets = ${setsValue}, work mins input = ${workMinsInputValue}, work secs input = ${workSecondsInputValue}`
  );

  //call the startWorkScreenCountown fn with these args
  startWorkScreenCountdown(
    setsValue,
    workMinsInputValue,
    workSecondsInputValue
  );
}
...
function startWorkScreenCountdown(sets, mins, secs) {
  ...
  console.log(
    `work countdown starts one second after ${timeWorkCountdownStarts}`
  );
...
function getAndShowWRestScreen() {
  ...
  console.log(
    `Sets = ${setsValue}, rest mins input = ${restMinsInputValue}, rest secs input = ${restSecondsInputValue}`
  );

  //call the startWorkScreenCountown fn with these args
  startRestScreenCountdown(
    setsValue,
    restMinsInputValue,
    restSecondsInputValue
  );
}
...
function startRestScreenCountdown(sets, mins, secs) {
  ...
  console.log(
    `rest countdown starts one second after ${timeRestCountdownStarts}`
  );
...
function editThisPreset() {
  ...
  console.log(
    `the edit btn of the preset under the name '${nameOfPresetToBeEdited}' has been clicked`
  );

  //then write this name into the "presetAboutToBeEdited" span which the handleEdit fn will use
  document.getElementById("presetAboutToBeEdited").innerHTML =
    nameOfPresetToBeEdited;
  ...
  let arrayFromLocalStorage = localStorage
    .getItem(`${nameOfPresetToBeEdited}`)
    .split(",");
...
function minusOneFromWorkPeriodEdit() {
  //if seconds > 0 minus one from the seconds, else minus one from the mins (and then reset the secs to 59)
  if (workSecondsInputEdit.value > 0) {
    workSecondsInputEdit.value--;
  } else {
...
function plusOneToWorkPeriodEdit() {
  //if seconds < 59, add one to the seconds, else add one to the mins (and then reset the secs to 0)
  if (workSecondsInputEdit.value < 59) {
    workSecondsInputEdit.value++;
  } else {
    workMinsInputEdit.value++;
...
function minusOneFromRestPeriodEdit() {
  //if seconds > 0 minus one from the seconds, else minus one from the mins (and then reset the secs to 59)
  if (restSecondsInputEdit.value > 0) {
    restSecondsInputEdit.value--;
  } else {
...
function plusOneToRestPeriodEdit() {
  //if seconds < 59, add one to the seconds, else add one to the mins (and then reset the secs to 0)
  if (restSecondsInputEdit.value < 59) {
    restSecondsInputEdit.value++;
  } else {
    restMinsInputEdit.value++;
...
function handleEdit(event) {
  ...
  let nameOfPresetToDeleteInThisEdit = document.getElementById(
    "presetAboutToBeEdited"
  ).innerHTML;
  console.log(`${nameOfPresetToDeleteInThisEdit}`);

  localStorage.removeItem(nameOfPresetToDeleteInThisEdit);
  document
    .getElementById(`${nameOfPresetToDeleteInThisEdit}SavedPreset`)
    .remove();

  //then add a new one with the current values
  savePresetToLocalStorage(
    presetNameEdit.value,
    setsEdit.value,
    workMinsInputEdit.value,
    workSecondsInputEdit.value,
    restMinsInputEdit.value,
...
function startThisPreset() {
  ...
  let thisPresetsSetsWorkAndRestValuesAsAString = localStorage.getItem(
    nameOfPresetToBeStarted
  );
  let arrayOfThisPresetsSetsWorkAndRestValues =
    thisPresetsSetsWorkAndRestValuesAsAString.split(",");
  ...
  workMinsInput.value = parseInt(
    arrayOfThisPresetsSetsWorkAndRestValues[1],
    10
  );
  workSecondsInput.value = parseInt(
    arrayOfThisPresetsSetsWorkAndRestValues[2],
    10
  );
  restMinsInput.value = parseInt(
    arrayOfThisPresetsSetsWorkAndRestValues[3],
    10
  );
  restSecondsInput.value = parseInt(
    arrayOfThisPresetsSetsWorkAndRestValues[4],
    10
  );
